...
== 主页 ==
 http://huangda1982.googlecode.com/

= 小时钟 变更履历 =
== 2010-06-09 ==
*V1.0更新内容*
 # 完全使用免费工具开发，去除了VC8生成的代码以及工程文件。
 # 删去了非QVGA机型支持。
 # 去除了WM5专用代码，WM5采用和WM2003相同的实现方式。
...
*使用的开发工具*
 ...
 # [http://cegcc.sourceforge.net/ mingw32ce]
 # [http://www.cygwin.com/ Cygwin]
----
== 2007-07-07 ==
*Bate3 (V0.84)更新内容*
 # 修正了-color参数指定颜色时的顺序问题。现在和主题文件中指定颜色的格式一样了，是#RRGGBB的形式，前一版本弄反了。
 # 制作了WM2003版，但是不能支持自动颜色功能。不指定颜色时默认是白色。
...
== 6.13 ==
*Bate2 (V0.83)更新内容*
 # 修正了非QVGA机型窗口显示不全的问题（想去掉窗口来着，因为意识到了实用性比显示我的名字等重要。不过实在不想改了）。
 # 修正了“-12”参数不能起作用的问题（如果想要继续使用Beta1版，可以用“-12h”参数，编码时手误了）。
...
== 6.7 ==
*Bate1 (V0.82)更新内容*
 # 使用了注册表通知，不再支持2003系统了。很抱歉，WM2003的朋友只能继续使用V0.81了。
 # 更名为《小时钟》。
 # 增加了自定义颜色和自动颜色功能（与任务栏前景色同色）。
...
*命令行参数举例*
 # 55#"\Program Files\TopClock\TopClock_QVGA.exe" -12 以自动颜色显示十二小时制的时间，启动时显示窗口。
 # 62#"\Program Files\TopClock\TopClock_QVGA.exe" -color # 0000ff -hide 以纯蓝色显示时间，启动时不显示窗口。
